...
def test_download():
    ...
    """
    MovieLens20M("./tmp_data", download=True)
    ...
    assert os.path.isdir("./tmp_data/MovieLens") is True

    # check if raw folder exists
    assert os.path.isdir("./tmp_data/MovieLens/raw") is True

    # check if processed folder exists
    assert os.path.isdir("./tmp_data/MovieLens/processed") is True

    # check if raw folder exists
    assert os.path.isdir("./tmp_data/MovieLens/raw") is True

    # check some random file from MovieLens
    assert os.path.isfile("./tmp_data/MovieLens/raw/{}/u.info".format(filename)) is True

    # check Readme file from MovieLens
    assert os.path.isfile("./tmp_data/MovieLens/raw/{}/README".format(filename)) is True

    # check if data file is not Nulll
    assert os.path.getsize("./tmp_data/MovieLens/raw/{}/u.data".format(filename)) > 0

    # check if info file is not Nulll
    assert os.path.getsize("./tmp_data/MovieLens/raw/{}/u.info".format(filename)) > 0
