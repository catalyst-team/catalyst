...
class GANRunner(MultiPhaseRunner):
    ...
    Various conditioning types, penalties and regularizations (such as WGAN-GP)
    ...
    def __init__(
        ...
        # TODO: simplify checks & transforms
        assert not ((d_fake_condition_keys is None) ^ (d_real_condition_keys is None))
        if d_fake_condition_keys is not None or d_real_condition_keys is not None:
            assert len(self.condition_keys) > 0
            assert all(
                key in self.condition_keys for key in d_fake_condition_keys
            ), "all discriminator conditions must be generator conditions"

        if (
            not self.condition_keys
            or d_real_condition_keys is None
        ):
            # simple case: no condition in discriminator
