...
class LMDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            sort (bool): If True then sort all sequences by length for efficient padding
            lazy (bool): If True then tokenize sequence in __getitem__ method 
                else will tokenize in __init__ also if set to true sorting is unavialible
        """
        ...
        if isinstance(tokenizer, str):
            ...
            raise TypeError(
                "tokenizer argument should be model name or huggingface pre trained tokenizer"
            )
        ...
        if not lazy:
            ...
            warnings.warn(
                "Warning! lazy set to True so we can't sort sequences by length.\n"
