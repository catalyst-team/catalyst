...
class RegionBasedMetric(ICallbackBatchMetric):
    ...
    def update(self, outputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        micro_metric = torch.mean(metrics_per_class)
        macro_metric = self.metric_fn(tp.sum(), fp.sum(), fn.sum())
        return metrics_per_class, micro_metric, macro_metric
    ...
    def update_key_value(
        ...
        """
        metrics_per_class, micro_metric, macro_metric = self.update(outputs, targets)
        # need only one time
