...
def test_iou():
    ...
    assert iou(empty, full, class_dim=1, mode="separately").item() == 0
    ...
    right[:, :, :, :half_size] = 0
    assert iou(left, right, class_dim=1, mode="separately").item() == 0

    # check 1: both empty, both full, complete overlap
    assert iou(empty, empty, class_dim=1, mode="separately").item() == 1
    assert iou(full, full, class_dim=1, mode="separately").item() == 1
    assert iou(left, left, class_dim=1, mode="separately").item() == 1

    # check 0.5: half overlap
    top_left = torch.zeros(shape)
    top_left[:, :, :half_size, :half_size] = 1
    assert torch.isclose(
        iou(top_left, left, class_dim=1, mode="separately"), torch.Tensor([[0.5]]),
    )
    ...
    assert torch.allclose(iou(a, b, class_dim=1, mode="separately"), ans)
    assert torch.allclose(iou(a, b, class_dim=1, mode="micro"), ans_micro)

    aaa = torch.cat([a, a, a], dim=0)
    bbb = torch.cat([b, b, b], dim=0)
    assert torch.allclose(iou(aaa, bbb, class_dim=1, mode="separately"), ans)
