...
def test_iou():
    ...
    assert criterion.iou(empty, full, activation="none").item() == 0
    ...
    right[:, :, :, :half_size] = 0
    assert criterion.iou(left, right, activation="none").item() == 0

    # check 1: both empty, both full, complete overlap
    assert criterion.iou(empty, empty, activation="none") == 1
    assert criterion.iou(full, full, activation="none") == 1
    assert criterion.iou(left, left, activation="none") == 1

    # check 0.5: half overlap
    top_left = torch.zeros(shape)
    top_left[:, :, :half_size, :half_size] = 1
    assert criterion.iou(top_left, left, activation="none").item() == 0.5
    ...
    assert torch.all(
        criterion.iou(a, b, classes=["dummy"], activation="none") == ans
    )
